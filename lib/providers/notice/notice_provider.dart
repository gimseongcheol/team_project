...
class NoticeProvider extends StateNotifier<NoticeState> with LocatorMixin {
  ...
  Future<void> updateNotice({
    ...
    try {
      ...
      NoticeModel noticeModel = await read<NoticeRepository>().updateNotice(
        files: files,
        desc: desc,
        title: title,
        uid: uid,
  ...
  Future<void> deleteNotice({
    ...
    try {
      await read<NoticeRepository>().deleteNotice(clubId: clubId, noticeModel: noticeModel);

      List<NoticeModel> newNoticeList = state.noticeList
          .where((element) => element.noticeId != noticeModel.noticeId)
          .toList();
